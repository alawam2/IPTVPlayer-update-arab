...
wget --no-check-certificate -O /tmp/e2iplayer-latest.zip "https://github.com/alawam2/IPTVPlayer-update-arab/raw/main/e2iplayer-latest.zip"

if [ ! -f /tmp/e2iplayer-latest.zip ]; then
...
if [ $? -ne 0 ]; then
    echo "ERROR: Unzip failed! ZIP file is corrupted."
    exit 1
fi
